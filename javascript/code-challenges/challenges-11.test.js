...
const totalSum = (input) => {
  ...
  return input.reduce((acc, value) => {
    acc += value.reduce((accumelator, val) => {
      accumelator += val;
      return accumelator;
    }, 0);
    return acc;
...
let findMaleAndFemale = (data) => {
  ...
  let names = result.reduce((acc,val)=>{
...
let findShortest = (data) => {
  // Solution code here...
  let short = starWarsData.reduce((acc,val)=>{
    if (parseInt(val.height) > parseInt(acc)) {
      return acc.toString();
    }else{
      acc = parseInt(val.height);
      return acc.toString();
    }
  },'');
  let char = data.filter(val =>{
    if(val.height === short){
      return true;
    }
  });
