...
const forLoopTwoToThe = (arr) => {
  ...
  for (let index = 0; index < arr.length; index++) {
    arr[index] = Math.pow(2, arr[index]);
  }
  return arr;
...
const charCode = (arr) => {
  // Solution code here...
  arr.map((element, index) => {
    arr[index] = element.charCodeAt(0);
  });
  return arr;
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  arr.map((element, index) => {
    arr[index] = element.ability.name;
  });
  return arr;
};
...
const extractStats = (arr) => {
  ...
  arr.map((element, index) => {
    arr[index]={};
    arr[index].name = element.stat.name;
    arr[index].total = element.effort+element.baseStat;
  });
  console.log(arr);
  return arr;
};
